...
import DivBox from '../../member_PC_HS/styleComponent/Signup/DivBox';
import FieldBtns from '../../member_PC_HS/styleComponent/Signup/FieldBtns';
...
export default function MypageEditFm() {
    ...
    useEffect(() => {
      const getProfile = async() => {
        ...
        setLogin(data.login);
        setSelectedButtons(data.field);
        setTextareaValue(data.intro);
    ...
    if(confirmed) {
      if(password1 !== password2) {
        ...
        const usersRef = doc(db, "financial_managers", params.id);
    ...
    const deleteBtn = async () => {
      ...
      if (confirmed) {
        try {
          ...
          await deleteDoc(doc(db, "financial_managers", params.id));
          navigate('/');
        } catch (error) {
    ...
    return (
      <div id = "layout" style = {{ overflow: 'hidden' }}>
        ...
        <form onSubmit = { handleSubmit }>
          ...
          <div style = {{ position:"absolute", top:"143px", left:"140px", fontSize:"0.9rem"}}>
            {
          ...
          <DivBox className='input-div'>
            ...
            {/* 자산관리사 자기소개 */}
            <DivBox className='input-div'>
              <label style={{paddingLeft: "100px"}}>Self - introduction</label>
              <div className='div-style'>
              <textarea
                style={{backgroundColor: "#E6E6E6", borderRadius: "40px", border: "none", padding: "20px"}}
                name="" 
                id="" 
                cols="57" 
                rows="5"
